...
<div class="wrap">
    ...
    <form id="kcseo-main-settings">
        ...
        <table width="40%" cellpadding="10" class="form-table">
            ...
            <tr>
                <th colspan="2"><h3><?php esc_html_e("Archive Page Schema", "wp-seo-structured-data-schema-pro") ?></h3></th>
            </tr>
            <tr>
                <th><?php esc_html_e("Archive?", "wp-seo-structured-data-schema-pro") ?></th>
                <td align="left" scope="row">
                    <?php $dd = !empty($settings['archive']) ? "checked" : null; ?>
                    <input type="checkbox" <?php echo $dd; ?> name="archive" value="1"
                           id="archive"><label
                            for="archive"><?php esc_html_e("Enable", "wp-seo-structured-data-schema-pro") ?></label>
                    <p class="description"><?php esc_html_e("This archive page is for blog post", "wp-seo-structured-data-schema-pro") ?></p>
                </td>
            </tr>
            <!--tr>
                <th><?php esc_html_e("Schema Type", "wp-seo-structured-data-schema-pro") ?></th>
                <td align="left" scope="row">
                    <select name="schema_type">
                        <option value=""><?php esc_html_e("Select one", "wp-seo-structure-data-schema-pro") ?></option>
                        <?php
                        $schema_type = [
                            'article' => esc_html__("Article", "wp-seo-structure-data-schema-pro"),
                            'news_article' => esc_html__("News Article", "wp-seo-structure-data-schema-pro"),
                            'blog_posting' => esc_html__("Blog Posting", "wp-seo-structure-data-schema-pro"),
                        ];

                        if ( !empty($schema_type) ) {
                            foreach ($schema_type as $key => $type) {
                                $slt = (!empty($settings['schema_type']) && $settings['schema_type'] == $key) ? " selected" : null;
            ...
            <tr>
                <th><?php esc_html_e("Product Archive?", "wp-seo-structured-data-schema-pro") ?></th>
                <td align="left" scope="row">
                    <?php $dd = !empty($settings['product_archive']) ? "checked" : null; ?>
                    <input type="checkbox" <?php echo $dd; ?> name="product_archive" value="1"
                           id="product_archive"><label
                            for="product_archive"><?php esc_html_e("Enable", "wp-seo-structured-data-schema-pro") ?></label>
                    <p class="description"><?php esc_html_e("This archive page is for WooCommerce and EDD product", "wp-seo-structured-data-schema-pro") ?></p>
                </td>
            </tr>
