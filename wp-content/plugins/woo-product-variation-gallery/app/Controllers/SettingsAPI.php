			...
			$response   = wp_remote_post( Licensing::$store_url,
				...
				if ( is_array($response) || 200 !== wp_remote_retrieve_response_code($response)) {
					...
					if ( is_wp_error( $response ) ) {
						...
						$err = __('An error occurred, please try again.', 'woo-product-variation-gallery');
					}
	
				   $message = !empty($err) ? $err : __('An error occurred, please try again.', 'woo-product-variation-gallery');
